...
string TLong::Output()
{
    ...
    for (int i=nmax-this->len+1;i<=nmax;i++){
        res += this->n[i] + '0';
        if (i==nmax-this->point && this->point!=0)
            res += ".";
        if (i==nmax-this->bracket && i<nmax)
            res += "(";
    }
    if (this->bracket!=0)
        res += ")";
...
TLong TLong::Power(TLong a,int b,int k)
{
    ...
    if (b<0)
    {
        ...
        r.len=1;
        r.sign='+';
        return Div_Dec(r, Power_Abs(a,-b));
    }else
